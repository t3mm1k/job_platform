...
class ResponseRepository(BaseRepository):
    ...
    def create(self, vacancy_id: str, user_id: int, message: str='') -> ResponseModel:
        self._users.require(user_id)
        self._vacancies.require(vacancy_id)
        response = ResponseModel(_id=str(ObjectId()), vacancy_id=vacancy_id, user_id=user_id, message=message, status='new', created_at=datetime.utcnow())
        self._collection.insert_one(response.model_dump(by_alias=True))
        return response

    def update_status(self, response_id: str, status: str, message: str='') -> ResponseModel:
        result = self._collection.update_one({'_id': response_id}, {'$set': {'status': status, 'message': message}})
