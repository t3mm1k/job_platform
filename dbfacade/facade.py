...
from .repositories import CompanyRepository, FavoriteRepository, ResponseRepository, ResumeRepository, UserRepository, VacancyRepository
from .models import CompanyModel, FavoriteModel, ResponseModel, ResumeModel, UserModel, VacancyModel
...
class DBFacade:
    ...
    def __init__(self, mongo_uri: str, db_name: str='job_platform') -> None:
        ...
        self._responses_repo = ResponseRepository(self._db['responses'], self._users_repo, self._vacancies_repo)
        self._favorites_repo = FavoriteRepository(self._db['favorites'], self._users_repo, self._vacancies_repo)
    ...
    def upsert_user(self, user: UserModel) -> UserModel:
        return self._users_repo.upsert(user)
    ...
    def list_companies(self) -> list[CompanyModel]:
        return self._companies_repo.list_all()
    ...
    def update_status_in_db(self, response_id: str, status: str, message: str='') -> ResponseModel:
    ...
    def list_favorite_vacancy_ids(self, user_id: int) -> list[str]:
        return self._favorites_repo.list_vacancy_ids_by_user(user_id)
