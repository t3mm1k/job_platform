...
import { fetchAllCompanies, setSelectedCompanyId } from "../store/slices/companySlice";
import * as api from "../api/client";
import { StorageKeys, UserRole } from "../models/dto";
...
const COUNT_ROLE_SELECTION = 3;
export default function Main() {
  const navigate = useNavigate();
  const selectedCompanyId = getSelectedCompanyId();
  let allCompanies = useSelector(state => state.company.allCompanies);
  let company = allCompanies.find(company => company._id === selectedCompanyId);
  if (company) {
    console.log('Компания есть туда сюда');
  }
  ...
  console.log(roleSelectionCount, "roleSelectionCount");
  const initialRoleConfirmed = sessionStorage.getItem(ROLE_SELECTION_CONFIRM_KEY) === 'true' || selectedCompanyId;
  const needsRoleSelectionInitially = !initialRoleConfirmed && roleSelectionCount < COUNT_ROLE_SELECTION;
  ...
  useEffect(() => {
    console.log("Location Processing Effect Triggered. ymapsReady:", ymapsReady, "locationProcessed:", locationProcessed.current, "initialLocationConfirmed:", initialLocationConfirmed, typeof initialLocationConfirmed);
    ...
    console.log(typeof initialLocationConfirmed);
    ...
    if (locationStage === 'idle' || locationStage === 'requesting') {
      ...
      if (coords && !autoCoords) {
        console.log("Геолокация получена:", coords);
        const currentCoords = [coords.longitude, coords.latitude];
        console.log("Обновление центра карты на:", currentCoords);
        dispatch(setCenter(currentCoords));
  ...
  const handleEmployerSelect = async () => {
    sessionStorage.setItem(ROLE_SELECTION_CONFIRM_KEY, "true");
    sessionStorage.setItem(StorageKeys.USER_ROLE, UserRole.EMPLOYER);
    ...
    try {
      ...
      try {
        ...
        alert('Вы можете изменить наименование компании в профиле.');
        navigate('/');
      } catch (error) {
  ...
  const handleJobSeekerSelect = () => {
    sessionStorage.setItem(ROLE_SELECTION_CONFIRM_KEY, "true");
    sessionStorage.setItem(StorageKeys.USER_ROLE, UserRole.JOB_SEEKER);
